...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view customView:(UIView *)customView modal:(BOOL)modal animated:(BOOL)animated
{
    ...
    hud.margin = 0;
    hud.opacity = 0.5;
    hud.userInteractionEnabled = modal;
    [hud show:animated];
    return MB_AUTORELEASE(hud);
}
...
{
    MBProgressHUD *hud = [[self alloc] initWithView:navigationController.view];
    UINavigationBar *navigationBar = navigationController.navigationBar;
    [navigationBar.superview insertSubview:hud belowSubview:navigationBar];
    [hud show:animated];
    return MB_AUTORELEASE(hud);
}
...
+ (BOOL)hideHUDBelowNavigationController:(UINavigationController *)navigationController animated:(BOOL)animated {
    MBProgressHUD *hud = [self HUDForView:navigationController.navigationBar.superview];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hide:animated];
        return YES;
